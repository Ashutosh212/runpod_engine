_base_ = [
    '../../../configs/_base_/datasets/custom_orientedformer_8cls_stage2.py',
...
# ── inverse-sqrt-frequency class weights (average = 1.0) ─────────────────────
# merged 3-source dataset annotation counts:
# arty:1407  camo:4989  logistic:13536  missile:14012  radar:10550
# smallvehicle:7186  tank:7607  vehicle:31897
class_weight = [2.210, 1.174, 0.713, 0.701, 0.807, 0.978, 0.951, 0.464]
...
# ── RepeatFactorSampler — upsample images with rare classes ──────────────────
# repeat_thr=0.02: arty ×1.78, radar ×1.24, others ≤1.0 (seen once per epoch)
...
# ── Fine-tune from stage-1 epoch_36 checkpoint ───────────────────────────────
# Classifier head shape mismatch (8 cls in ckpt vs 8 cls here) — exact match,
# weights load cleanly with no re-initialisation needed.
# load_from = '/sfs/ai4rs/projects/OrientedFormer/orientedformer_le90_r50_q300_layer2_head64_point32_1x_dotav1.0-ms.pth'
load_from = '/sfs/work_dirs/orientedformer_custom_8cls_stage2/orientedformer_custom_8cls_may_w3/epoch_6.pth'
# ── optimizer — lower LR for stage-2 fine-tuning ─────────────────────────────
optim_wrapper = dict(
    optimizer=dict(
        _delete_=True, type='AdamW', lr=2e-5, weight_decay=1e-6),
    clip_grad=dict(max_norm=1, norm_type=2))

# ── 36 epochs, validate every 3 epochs ───────────────────────────────────────
train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=36, val_interval=3)

param_scheduler = [
    dict(type='LinearLR', start_factor=1.0 / 3,
...
default_hooks = dict(
    checkpoint=dict(
        ...
        save_best=None,
        max_keep_ckpts=30,
        out_dir='/sfs/work_dirs/orientedformer_custom_8cls_stage2'))
